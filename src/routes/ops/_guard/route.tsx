      ...
      <aside className="fixed inset-y-0 left-0 z-30 w-72 border-r border-slate-200/80 bg-white/70 backdrop-blur-xl shadow-sm">
        <div className="flex h-20 items-center gap-3 border-b border-slate-200/60 px-6">
          <div className="relative shrink-0">
            <div className="h-11 w-11 rounded-xl bg-gradient-to-br from-amber-500 via-orange-600 to-rose-600 flex items-center justify-center shadow-lg shadow-amber-500/30 transition-transform hover:scale-110 hover:rotate-3">
                  ...
                  return (
                    <Link key={item.to} to={item.to}
                      className={cn("group relative flex items-center gap-3 rounded-lg text-sm transition-all px-3 py-2.5",
                        active ? "bg-gradient-to-r from-amber-50 to-orange-50 text-amber-700 font-semibold shadow-sm" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900")}>
                      {active && <span className="absolute left-0 top-1/2 -translate-y-1/2 h-6 w-1 rounded-r-full bg-gradient-to-b from-amber-500 to-orange-600" />}
                      <Icon className={cn("h-[18px] w-[18px] shrink-0", active ? "text-amber-600" : "text-slate-400 group-hover:text-slate-600 group-hover:scale-110")} />
                      <span className="flex-1">{item.label}</span>
                      {active && <ChevronRight className="h-3.5 w-3.5 text-amber-500" />}
                    </Link>
                  );
                })}
              </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 border-t border-slate-200/60 bg-white/60 backdrop-blur-xl p-3">
          <div className="flex items-center gap-3 rounded-xl p-3 border bg-gradient-to-r from-slate-50 to-slate-100/60 border-slate-200/60">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11px] text-slate-500">Opérations</div>
            </div>
        ...
        <header className="sticky top-0 z-20 border-b border-slate-200/60 bg-white/70 backdrop-blur-xl">
          <div className="flex h-16 items-center justify-between px-8 gap-4">
            <nav className="flex items-center gap-1.5 text-sm min-w-0">
              <Link to="/ops" className="p-1 rounded hover:bg-slate-100 text-slate-400"><Home className="h-3.5 w-3.5" /></Link>
            ...
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500 to-orange-600 text-white text-xs font-medium shadow-sm">
              <Wrench className="h-3.5 w-3.5" /> Espace opérations
            </div>
          </div>
        </header>
