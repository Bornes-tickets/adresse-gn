...
function AdminBeacons() {
  ...
  const muterGen = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(
        `Lot ${r.lotCode} créé : ${r.quantite} balises (${r.premier} → ${r.dernier}).`,
      );
      setOuvrirGen(false);
      void qc.invalidateQueries({ queryKey: ["admin", "beacons"] });
  ...
  const muterAffect = useMutation({
    mutationFn: () =>
      affecter({ data: { lotId: affectLot, agentId: affectAgent } }),
    onSuccess: () => {
  ...
  const exportEnCours =
    muterExport.isPending || muterZip.isPending || muterCsv.isPending;
  const etapeExport = muterExport.isPending
    ? "PDF"
    : muterZip.isPending
      ? "ZIP PNG"
      : "CSV";
  ...
  /* KPI calculs — visibles pour la page courante (info) */
  const rows = balises.data?.rows ?? [];
  const kpis = useMemo(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                Génération, suivi et export des balises Adresse GN.
              </p>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => void balises.refetch()}
            disabled={balises.isFetching}
          >
            <RefreshCw
              className={cn("size-4", balises.isFetching && "animate-spin")}
            />
            Actualiser
          </Button>
          <Button
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
        <Kpi
          label="Total"
          valeur={kpis.total}
          aide="Balises correspondant aux filtres"
          icone={Package}
          ton="slate"
        />
        <Kpi
          label="Actives"
          valeur={kpis.actives}
          aide="En service sur le terrain"
          icone={CheckCircle2}
          ton="emerald"
        />
        <Kpi
          label="Générées"
          valeur={kpis.generees}
          aide="En attente d'installation"
          icone={Sparkles}
          ton="sky"
        />
        <Kpi
          label="Suspendues"
          valeur={kpis.suspendues}
          aide="Temporairement bloquées"
          icone={PauseCircle}
          ton="amber"
        />
        <Kpi
          label="Annulées"
          valeur={kpis.annulees}
          aide="Retirées définitivement"
          icone={Ban}
          ton="rose"
        />
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              <Label className="text-xs text-slate-600">Recherche par numéro</Label>
          ...
          {/* Barre d'export si un lot est sélectionné */}
          {lotId !== "tous" && (
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-medium text-slate-900">
                    Génération {etapeExport} en cours…
                  </p>
                  <p className="text-xs text-slate-500">
                    Le fichier sera téléchargé automatiquement dès qu'il est prêt.
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-5 px-1 py-5 text-sm">
              ...
              <div className="rounded-xl bg-gradient-to-br from-slate-900 to-slate-700 p-5 text-white">
                ...
                <p className="mt-1 font-mono text-2xl font-bold">
                  {fiche.data.beacon.public_number}
                </p>
                <div className="mt-3">
              ...
              <Section titre="Métadonnées" icone={InfoIcon}>
                <div className="grid grid-cols-2 gap-3">
                  <Info
                    label="Lot"
                    valeur={(fiche.data.beacon as any).lots?.code ?? "—"}
                    mono
                  />
                  <Info
                    label="Statut"
                    valeur={statusLabel(fiche.data.beacon.status)}
                  />
                  <Info
                    label="Créée le"
                    valeur={formatDateFr(fiche.data.beacon.created_at)}
                  />
                  <Info
                    label="Activée le"
                    valeur={formatDateFr(fiche.data.beacon.activated_at)}
                  />
                </div>
              </Section>
      ...
      <Dialog open={ouvrirAffect} onOpenChange={setOuvrirAffect}>
        <DialogContent className="max-h-[90dvh] w-[calc(100vw-2rem)] max-w-lg overflow-y-auto sm:w-full">
          <DialogHeader>
            ...
            <DialogDescription>
              L'agent recevra les balises de ce lot dans son espace de travail.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
