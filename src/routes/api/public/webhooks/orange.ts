/** Webhook Orange Money (§8.4). Signature HMAC vérifiée avant tout traitement. */
import { createFileRoute } from "@tanstack/react-router";

export const Route = createFileRoute("/api/public/webhooks/orange")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const provider = getProviderRaw("orange");
        ...
        const resultat = await appliquerEvenementProvider(
          webhookId,
          "orange",
          verification.event,
        );
